...
#         alm_fun.show_msg(runtime['log'],1, runtime['job_name'] + ' started......')
        ...
        if alm_predictor.type > 0:                    
            ...
            #update alogrithm-level hyperparameter
            print('******Algorithm level hyperparameters*****')
            for cur_hp in alm_predictor.hyperparameter.keys():
                if alm_predictor.hyperparameter[cur_hp]['hp_type'] == 3:
                    print(str(cur_hp) + ': ' + str(cur_hp_dict[cur_hp]))                                                                                
                    setattr(alm_estimator.estimator,cur_hp,cur_hp_dict[cur_hp])
            print('******************************************')                            
...
#         alm_fun.show_msg(runtime['log'],1, runtime['job_name'] + ' started to predict......')
        r = alm_estimator.run(features, alm_dataset.dependent_variable, alm_predictor.ml_type, cur_train_df, cur_test_df ,extra_data_df,None,alm_predictor,model_file,runtime['balance_weights'])
...
#                             core_data.loc[core_data['set_name'].isin(set_list),'weight'] = core_weight
            ...
            if alm_predictor.type == 2:  # Naive type set the weight of all training examples to 1 
                core_data['weight'] = 1
                extra_data['weight'] = 1
            if alm_predictor.type == 3:  # Naive type set the weight of all training examples to 1 
                core_data['weight'] = 1
                extra_data['weight'] = 0
    ...
    def save_best_hp_dict_from_trials(self,runtime) :
        ...
        if runtime['trials_max_num'] == -1:
            trials_max_num = alm_predictor.hyperopt_trials
        else:
            trials_max_num = runtime['trials_max_num']            
        ...
        cur_trials_df = cur_trials_df.loc[cur_trials_df['trial'] < trials_max_num,:]
...
#                              '#800000', '#800080', '#333333', '#4ca3dd','#ff00ff','#008000','#0e2f44','#daa520','#444444','#555555']
        ...
        color_dict['MutPred'] = '#008000'
        color_dict['VARITY_R_LOO'] = '#666666'
        color_dict['VARITY_ER_LOO'] = '#777777'
        color_dict['eve'] = '#008000'
...
#                 predictor_name = predictor.split('_')[0]
            if ('_CV' in predictor):            
                if ('VARITY' in predictor):
                    predictor_name = predictor[:-3]                                           
                else:
                    if ('_ER_CV' in predictor):
                        predictor_name = predictor[:-6]
                    if ('_R_CV' in predictor):
                        predictor_name = predictor[:-5]                                                
            else:
...
#                 all_dict[predictor]['color'] = color_lst[color_index]
                ...
                for cur_fold in range(cv_folds):                 
                    ...
                    #interpret precision recall curve 
                    ...
                    #original precision recall curve and ROC curve                   
                    ...
                    #interpret precision recall curve or ROC curve                                     
            ...
            for score_type in score_types:
                ...
                for score_metric in score_metrics:
                    ...
                    if run_bootstrapping == 1:                               
                        ...
                        ax.hist(diff,100)
                        fig.tight_layout()
                        plt.savefig(output_file + '_' + predictor + '_hist.png')  
...
#                         all_dict[predictor]['result'][score_type + '_' + score_metric +'_display'] =  "{:.3f}".format(all_dict[predictor]['result'][score_type + '_' + score_metric + '_mean']) + '±' + "{:.3f}".format(all_dict[predictor]['result'][score_type + '_' + score_metric + '_se']) + ' (' + all_dict[predictor]['result'][score_type + '_' + score_metric +'_ci'] +',' + "{:.2e}".format(all_dict[predictor]['result'][score_type + '_' + score_metric + '_pvalue']) +')'
        ...
        display_columns = ['predictor']
        for plot_metric in plot_metric_orders:
             display_columns.append(plot_metric + '_display')
        pass
        score_output[display_columns].to_csv(output_file.replace('img','csv') + '_display.csv',index = False)
...
#         correlation_output_df = correlation_output_df.drop_duplicates()  
    ...
    def plot_data_quantity(self,runtime):
        ...
        all_data = pd.concat([train_data,extra_data])
        
        
#         extra_train_clinvar_positive = extra_data.loc[(extra_data['train_clinvar_source'] == 1) & (extra_data['label'] == 1)]
#         extra_train_clinvar_positive.to_csv(self.project_path + 'output/csv/extra_train_clinvar_positive.csv',index = False)
#         extra_train_clinvar_negative  = extra_data.loc[(extra_data['train_clinvar_source'] == 1) & (extra_data['label'] == 0)]
#         extra_train_clinvar_negative.to_csv(self.project_path + 'output/csv/extra_train_clinvar_negative.csv',index = False)
#         extra_train_gnomad = extra_data.loc[extra_data['train_gnomad_source'] == 1]
#         extra_train_gnomad.to_csv(self.project_path + 'output/csv/extra_train_gnomad.csv',index = False)
#         extra_train_hgmd = extra_data.loc[extra_data['train_hgmd_source'] == 1]   
#         extra_train_hgmd.to_csv(self.project_path + 'output/csv/extra_train_hgmd.csv',index = False)
#         extra_train_humsavar_positive = extra_data.loc[(extra_data['train_humsavar_source'] == 1) & (extra_data['label'] == 1)]
#         extra_train_humsavar_positive.to_csv(self.project_path + 'output/csv/extra_train_humsavar_positive.csv',index = False)
#         extra_train_humsavar_negative = extra_data.loc[(extra_data['train_humsavar_source'] == 1) & (extra_data['label'] == 0)]
#         extra_train_humsavar_negative.to_csv(self.project_path + 'output/csv/extra_train_humsavar_negative.csv',index = False)
#         extra_train_mave_positive = extra_data.loc[(extra_data['train_mave_source'] == 1) & (extra_data['label'] == 1)]
#         extra_train_mave_positive.to_csv(self.project_path + 'output/csv/extra_train_mave_positive.csv',index = False)        
#         extra_train_mave_negative = extra_data.loc[(extra_data['train_mave_source'] == 1) & (extra_data['label'] == 0)]
#         extra_train_mave_negative.to_csv(self.project_path + 'output/csv/extra_train_mave_negative.csv',index = False)
# 
#         

        #************************************************************        
        #The quantity of different data resources
        #************************************************************
        clinvar_train_data = all_data.loc[(all_data['train_clinvar_source'] == 1) & all_data['label'].isin([0,1]) ,:]


        data_name_labels = {}        
        data_name_labels['humsavar'] = 'HumsaVar'
        data_name_labels['gnomad'] = 'gnomAD \n(observed as homozygous)'
        data_name_labels['hgmd'] = 'HGMD (2015)'
        data_name_labels['mave'] = 'MaveDB'
       
        af_cutoff = 0.005
        star_cutoff = 0
       
        all_train_data_quantity = pd.DataFrame()
        for data_name in ['clinvar','humsavar','hgmd','gnomad','mave']:
            if data_name == 'clinvar':
#                 all_train_data_quantity.loc['ClinVar \n(rare,review stars <' +  str(star_cutoff) + ')','total'] =  all_data.loc[(train_data['train_' + data_name.lower() + '_source'] == 1) & train_data['label'].isin([0,1]) & (train_data['clinvar_review_star'] <star_cutoff) & (train_data['gnomAD_exomes_AF'] <= af_cutoff) ,:].shape[0]
#                 all_train_data_quantity.loc['ClinVar \n(rare,review stars <' +  str(star_cutoff) + ')','bottom'] = all_data.loc[(train_data['train_' + data_name.lower() + '_source'] == 1) & train_data['label'].isin([0]) & (train_data['clinvar_review_star'] <star_cutoff) & (train_data['gnomAD_exomes_AF'] <= af_cutoff) ,:].shape[0]
#                 all_train_data_quantity.loc['ClinVar \n(rare,review stars <' +  str(star_cutoff) + ')','up'] = all_data.loc[(train_data['train_' + data_name.lower() + '_source'] == 1) & train_data['label'].isin([1]) & (train_data['clinvar_review_star'] <star_cutoff) & (train_data['gnomAD_exomes_AF'] <= af_cutoff) ,:].shape[0]
#                 
                all_train_data_quantity.loc['ClinVar \n(common,MAF >' +  str(af_cutoff) + ')','total'] =  all_data.loc[(all_data['train_' + data_name.lower() + '_source'] == 1) & all_data['label'].isin([0,1]) & (all_data['gnomAD_exomes_AF'] > af_cutoff) ,:].shape[0]
                all_train_data_quantity.loc['ClinVar \n(common,MAF >' +  str(af_cutoff) + ')','bottom'] = all_data.loc[(all_data['train_' + data_name.lower() + '_source'] == 1) & all_data['label'].isin([0])  & (all_data['gnomAD_exomes_AF'] > af_cutoff) ,:].shape[0]
                all_train_data_quantity.loc['ClinVar \n(common,MAF >' +  str(af_cutoff) + ')','up'] = all_data.loc[(all_data['train_' + data_name.lower() + '_source'] == 1) & all_data['label'].isin([1]) & (all_data['gnomAD_exomes_AF'] > af_cutoff) ,:].shape[0]                
            else:
                all_train_data_quantity.loc[data_name_labels[data_name],'total'] =  all_data.loc[(all_data['train_' + data_name.lower() + '_source'] == 1) & all_data['label'].isin([0,1]) ,:].shape[0]
                all_train_data_quantity.loc[data_name_labels[data_name],'bottom'] = all_data.loc[(all_data['train_' + data_name.lower() + '_source'] == 1) & all_data['label'].isin([0]) ,:].shape[0]
                all_train_data_quantity.loc[data_name_labels[data_name],'up'] = all_data.loc[(all_data['train_' + data_name.lower() + '_source'] == 1) & all_data['label'].isin([1]) ,:].shape[0]

            
            positive_num =  all_data.loc[(all_data['train_' + data_name.lower() + '_source'] == 1) & all_data['label'].isin([1])  ,:].shape[0]
            rare_negative_num = all_data.loc[(all_data['train_' + data_name.lower() + '_source'] == 1) & all_data['label'].isin([0]) & (all_data['gnomAD_exomes_AF'] < 0.005) ,:].shape[0]
            common_negative_num = all_data.loc[(all_data['train_' + data_name.lower() + '_source'] == 1) & all_data['label'].isin([0]) & (all_data['gnomAD_exomes_AF'] >= 0.005) ,:].shape[0]
                                            
            print  (data_name + ' positive: '  + str(positive_num))
            print  (data_name + ' Commom negative: '  + str(common_negative_num))
            print  (data_name + ' Rare negative: '  + str(rare_negative_num))
                            
        #************************************************************   
        #Cinvar data with different AF cutoffs
        #************************************************************                
        clinvar_train_data = train_data.loc[(train_data['train_clinvar_source'] == 1) & train_data['clinvar_label'].isin([0,1]) & (train_data['clinvar_review_star'] >= star_cutoff) ,:]
        clinvar_train_data_quantity = pd.DataFrame()
        for maf_cutoff in [0.000001,0.00001,0.0001,0.001,0.005]:
            if maf_cutoff == 0:
                index = 'MAF =' + str(maf_cutoff)
            else:
                index = 'MAF <=' + str(maf_cutoff)
            clinvar_train_data_quantity.loc[index,'total'] = clinvar_train_data.loc[clinvar_train_data['gnomAD_exomes_AF'] <= maf_cutoff,:].shape[0]
            clinvar_train_data_quantity.loc[index,'bottom'] = clinvar_train_data.loc[(clinvar_train_data['gnomAD_exomes_AF'] <= maf_cutoff) & (clinvar_train_data['clinvar_label'] == 0),:].shape[0]
            clinvar_train_data_quantity.loc[index,'up'] = clinvar_train_data.loc[(clinvar_train_data['gnomAD_exomes_AF'] <= maf_cutoff) & (clinvar_train_data['clinvar_label'] == 1),:].shape[0]
        ...
        ylim_max = 60000  
        title = 'ClinVar rare missense variants with high quality pathogenicity annotation (AF <=' + str(af_cutoff) + ', review stars >=' +str(star_cutoff) + ')'
        title = 'ClinVar rare missense variants with high quality pathogenicity annotation'
        alm_fun.plot_stacked_barplot_ax(ax,clinvar_train_data_quantity, title, title_size, x_label, y_label, label_size, tick_size, ylim_min, ylim_max,legend_size)             
        ax = plt.subplot(2,1,2)
        ylim_max = 60000  
        title = 'Other missense variants in different databases'        
        alm_fun.plot_stacked_barplot_ax(ax,all_train_data_quantity, title, title_size, x_label, y_label, label_size, tick_size, ylim_min, ylim_max,legend_size)

        fig.tight_layout()
        fig.subplots_adjust(hspace = 0.4)      
        plot_name = self.project_path + 'output/img/clinvar_data_quantity.png'        
        ...
        #                 cur_data_df.loc[cur_data_df['set_name'].isin(qip_set_list),cur_qip + '_weight'] = cur_qip_weight
                ...
                if plot_weight_type == 0:  
                    ...
                    if plot_qip_col == 'gnomAD_exomes_nhomalt':
                        # ax.scatter(range(1,cur_data_df.shape[0]+1),np.log10(cur_data_df[plot_qip_col]))
